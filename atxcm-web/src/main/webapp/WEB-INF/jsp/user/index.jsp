        ...
        <script type="text/javascript">
            ...
            $(function () {
			    ...
			    $("#queryBtn").click(function(){
			    	var queryText = $("#queryText").val();//获取输入框的值
			    	if ( queryText == "" ) {
			    		likeflg = false;//决定需不需要模糊查询
			    	} else {
			    		likeflg = true;
			    	}
            ...
            function pageQuery( pageno ) {
            	...
            	if ( likeflg == true ) {
            		jsonData.queryText = $("#queryText").val();//如果输入框里有文字就加入json中
            	}
            	...
            	$.ajax({
            		...
            		success : function(result) {
            			...
            			if ( result.success ) {
            				// 局部刷新页面数据
            				var tableContent = "";//table内容
            				var pageContent = "";//分页内容
            				
            				var userPage = result.data;
            				var users = userPage.datas;//用户的集合
            				//$.each遍历数组的意思
            				//goUpdatePage('+user.id+')用来编辑用户
            				$.each(users, function(i, user){
            	                tableContent += '<tr>';
	          	                tableContent += '  <td>'+(i+1)+'</td>';
	          					tableContent += '  <td><input type="checkbox" name="userid" value="'+user.id+'"></td>';//复选框的value赋值为用户的id
            				...
            				if ( pageno < userPage.totalno ) {//userPage.totalno总页码
            ...
            function deleteUsers() {
            	var boxes = $("#userData :checkbox");//获取选择多少个复选框
            	if ( boxes.length == 0 ) {
        			...
        			layer.confirm("删除选择的用户信息, 是否继续",  {icon: 3, title:'提示'}, function(cindex){
        				...
        				$.ajax({
        					...
        					data : $("#userForm").serialize(),//将表格中的内容序列化
        					success : function(result) {
        						if ( result.success ) {
        							pageQuery(1);//删除完重新查一遍数据显示
        						} else {
